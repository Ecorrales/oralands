 ...
 */
export type Baselines = Partial<typeof B>;

export function computeTitle(s: Partial<CharStats>, level: number, player: string, levelsInStretch = level, baselines?: Baselines | null): AwardedTitle {
  const B = { ...DEFAULT_B, ...(baselines ?? {}) };   // baselines reales (de la comunidad) pisan los de fábrica
