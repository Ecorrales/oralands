...
const TEMPLATES: Template[] = [
  { name: "Esqueleto",    tags: ["undead"],            ch: { strength: 4, vitality: 4, dexterity: 4, intelligence: 3 }, weapon: { name: "rusty sword", damage: "1d5", accuracy: "1d4", abilities: ["stab"] } },
  { name: "Rata gigante", tags: ["animal", "rodent"],  ch: { strength: 2, vitality: 3, dexterity: 5, intelligence: 1 }, weapon: { name: "teeth", damage: "1d3", accuracy: "1d5", abilities: ["quick_stab"] } },
  { name: "Lobo",         tags: ["animal"],            ch: { strength: 4, vitality: 4, dexterity: 6, intelligence: 2 }, weapon: { name: "fangs", damage: "1d5", accuracy: "2d5", abilities: ["cut"] } },
];

/** depth = índice de sala (0-based) escala características; level = etapa (nivel del enemigo). */
export function makeDungeonEnemy(depth: number, level = 1): Creature {
...
/** Grupo de enemigos para una sala. depth escala características; level = etapa. */
export function makeDungeonGroup(depth: number, stage: number): Creature[] {
  const n = rollGroupSize(stage);
  const group: Creature[] = [];
  const seen: Record<string, number> = {};
  for (let i = 0; i < n; i++) {
    const e = makeDungeonEnemy(depth, stage);
