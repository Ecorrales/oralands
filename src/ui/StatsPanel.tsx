...
export function StatsPanel({ player, points, onSpend, onClose }: {
  player: Creature; points: number;
  onSpend: (k: keyof Characteristics) => void; onClose: () => void;
}) {
  return (
    <div className="overlay" onClick={onClose}>
      <div className="statspanel" onClick={(e) => e.stopPropagation()}>
        <div className="cap">{player.name} · Nivel {player.level}</div>
        <StatsInline player={player} points={points} onSpend={onSpend} />
